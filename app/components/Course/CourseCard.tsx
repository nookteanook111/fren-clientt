...
const CourseCard: FC<Props> = ({ item, isProfile, expireIn = null }) => {
  ...
  let expireDateText = ''
  if (expireIn !== null) {
    expireDateText = dayjs(dayjs().add(+expireIn, 'day').toDate()).format(
      'DD/MM/YYYY',
    )
  }
  return (
      ...
      <div className="w-full max-w-[300px] border-2 md:h-[initial] md:min-h-[35vh] bg-white flex flex-col md:block   dark:bg-slate-500 dark:bg-opacity-20 hover:scale-95 duration-300 rounded-lg md:p-3 p-1">
        ...
        <div className="hidden md:block">
          ...
          <div className="w-full flex flex-col md:flex-row items-center justify-between">
            ...
            <h5
              className={`text-black dark:text-[#fff] text-[18px] md:text-[12px] ${
            ...
            <div className="flex items-center pb-3">
              <h5 className="text-[12px] text-black dark:text-[#fff] mt-2">
                {expireIn > 0 ? (
                  <span>
                    {' '}
                    หมดอายุในอีก {expireIn} วัน{' '}
                    <span className="text-gray-500">{expireDateText}</span>
                  </span>
                ) : (
                  <span className="text-[red]"> หมดอายุแล้ว</span>
                )}
              </h5>
            </div>
          ) : (
          ...
          <div>
            <Link
              ...
              className="mt-4 flex"
            >
              <button className="w-full bg-secondary py-2 sm:py-2 lg:py-4 rounded-xl text-white cursor-pointer hover:bg-primary duration-100 text-[12px] sm:text-[12px] lg:text-[16px]">
                เลือกซื้อคอร์สเรียน
              </button>
            </Link>
          </div>
        ...
        <div className="block md:hidden p-3 w-full relative ">
          ...
          <div className="flex justify-between">
            <div className="w-full flex flex-col md:flex-row text-[#777]">
              <Ratings rating={item.ratings} />
              <h5
                className={`text-[#777] text-[10px] md:text-[12px] ${
                  isProfile && 'hidden 800px:inline'
                }`}
              >
